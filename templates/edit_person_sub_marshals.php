...
$query_comb = "SELECT id_combat, name_combat, cn, ea, note FROM Combat LEFT JOIN"
        . "(SELECT  card_marshal as cn, expire_marshal as ea, id_combat as ic,"
...
$marshals = mysqli_query ($cxn, $query_marshals) 
        or die ("Couldn't execute query to find known/current marshal's warrantss.");
...
while ($row = mysqli_fetch_assoc($marshals)){
    ...
    if ($curr_id_combat!= $id_combat) {// Build for the next item in combats
        ...
        echo "<tr><td class='text-center' width='25%'><strong>$name_combat</strong><br>"
                . "expires:<input type='date' class='date' id='expire_marshal_$id_combat' "
                . "name='dynmdate[]' value ='".$mcombat["ea"]."'><br>"
                . "card number:<input type='number' name='dynmcard[]' value='"
                . $mcombat["cn"]."'id='card_number_$id_combat' >"
                . "Note:<br><textarea name='dynmnote[]' rows='2' cols='10'>".$mcombat["note"]."</textarea>"
                . "</td>";
    }
