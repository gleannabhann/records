...
extract($event_info);
if ($id_site<0) {
    if (DEBUG) {echo "Event record doesn't include Site ID<p>";}
    $name_site="Unknown";
} else {
...
$data = [':id_event' => $id_event];
if (DEBUG) { echo "<p>The Recipients Query is:<p>$query</p><p>Vars:<br>"; print_r($data); echo "</p>";}
try {
  $sth = $cxn->prepare($query);
  $sth->execute($data);
...
/* Now let's list potential recipients for the herald's eyes only */
    ...
    if (DEBUG){echo "Potential Recipients query is:<p>$query</p><p>vars are:<br>"; print_r($data); echo "</p>";}
    try {
    $sth = $cxn->prepare($query);
    $sth->execute($data);
    } catch (PDOException $e) {
